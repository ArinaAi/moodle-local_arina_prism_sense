 ...
 */
function local_arina_prism_sense_upload_slides_to_azure($slideFiles, $previewId, $containerName)
{
    try {
        ...
        foreach ($slideFiles as $slideFile) {
            ...
            // Upload to Azure
            $result = local_arina_prism_sense_upload_to_azure_blob(
                $accountName,
                $containerName,
                $accountKey,
                $blobName,
 ...
 */
function local_arina_prism_sense_download_slides_from_azure($previewId, $containerName)
{
    try {
        $accountName = AZURE_STORAGE_ACCOUNT_NAME;
        ...
        for ($i = 0; $i < 100; $i++) {
            ...
            // Generate Azure URL
            $azureUrl = local_arina_prism_sense_get_azure_blob_url($accountName, $containerName, $blobName);

            // Check if blob exists
            if (local_arina_prism_sense_check_azure_blob_exists($azureUrl)) {
                $images[] = [
                    'filename' => $filename,
 ...
 * @return bool True if successful
 */
function local_arina_prism_sense_upload_to_azure_blob(
    $accountName,
    $containerName,
    $accountKey,
    $blobName,
 ...
 */
function local_arina_prism_sense_get_azure_blob_url($accountName, $containerName, $blobName)
{
 ...
 */
function local_arina_prism_sense_check_azure_blob_exists($blobUrl)
{
    $ch = curl_init($blobUrl);
 ...
 */
function local_arina_prism_sense_count_slide_regenerations()
{
    $versions = local_arina_prism_sense_get_slide_versions();
 ...
 */
function local_arina_prism_sense_get_latest_slide_version()
{
    $versions = local_arina_prism_sense_get_slide_versions();
    if (empty($versions)) {
        return null;
 ...
 */
function local_arina_prism_sense_restore_slide_version($contentid, $timestamp, $containerName)
{
    $previewId = 'content_' . $contentid . '_' . $timestamp;
    return local_arina_prism_sense_download_slides_from_azure($previewId, $containerName);
}
 ...
 */
function local_arina_prism_sense_generate_blob_sas_token($accountName, $containerName, $blobName, $accountKey)
{
 ...
 */
function local_arina_prism_sense_parse_azure_blob_list_response($response)
{
    $maxCount = -1;
    try {
 ...
 */
function local_arina_prism_sense_check_folder_has_generated_content($accountName, $accountKey, $containerName, $prefix)
{
    $hasContent = false;
    $response = local_arina_prism_sense_execute_azure_blob_list_call(
        $accountName,
        $accountKey,
        $containerName,
        $prefix
        ...
        try {
            ...
            if (isset($xml->Blobs->Blob)) {
                foreach ($xml->Blobs->Blob as $blob) {
                    ...
                    if (
                        strpos($name, '/lecture_content.json') !== false ||
 ...
 */
function local_arina_prism_sense_get_azure_regen_count($courseid, $sectionid)
{
    $regenCount = 0;
    ...
    if (defined('AZURE_STORAGE_ACCOUNT_NAME') && defined('AZURE_STORAGE_ACCOUNT_KEY')) {
        ...
        $containerName = defined('AZURE_BLOB_CONTAINER_NAME') ? AZURE_BLOB_CONTAINER_NAME : 'arina_prism_sense';
        ...
        $nextCount = 0;
        if ($response) {
            $nextCount = local_arina_prism_sense_parse_azure_blob_list_response($response);
        }
        ...
        if ($latestIndex >= 0) {
            ...
            $hasContent = local_arina_prism_sense_check_folder_has_generated_content(
                $accountName,
                $accountKey,
                $containerName,
                $latestFolderPrefix
